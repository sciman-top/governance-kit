...
if ($null -ne $projectRulePolicy.defaults) {
  ...
  if ($null -ne $projectRulePolicy.defaults.PSObject.Properties['enforce_boundary_class']) {
    [void](Validate-BooleanValue -Value $projectRulePolicy.defaults.enforce_boundary_class -Message "[CFG] project-rule-policy.defaults.enforce_boundary_class must be boolean")
  }
...
$repoRoots = @()
foreach ($repoRoot in @($repos)) {
  if (-not [string]::IsNullOrWhiteSpace([string]$repoRoot)) {
    $repoRoots += Normalize-Repo ([string]$repoRoot)
  }
}
...
foreach ($t in $targets) {
  ...
  if (-not [System.IO.Path]::IsPathRooted(($target -replace '/', '\'))) {
    ...
    $expectedBoundaryClass = Get-ExpectedBoundaryClass -Source ([string]$t.source)
    $boundaryClassProp = $t.PSObject.Properties['boundary_class']
    ...
    if ($enforceBoundaryClass -and $expectedBoundaryClass -eq "unknown") {
      Write-Host "[CFG] unknown source layer for boundary classification: source=$($t.source) target=$($t.target)"
      $fail++
    } elseif ($enforceBoundaryClass) {
      if ([string]::IsNullOrWhiteSpace($boundaryClassText)) {
        Write-Host "[CFG] missing boundary_class: source=$($t.source) target=$($t.target)"
        $fail++
      } elseif (-not (Test-BoundaryClassValue -BoundaryClass $boundaryClassText)) {
        Write-Host "[CFG] invalid boundary_class: expected one of global-user/project/shared-template source=$($t.source) target=$($t.target) actual=$boundaryClassText"
        $fail++
      } elseif ($boundaryClassText -ne $expectedBoundaryClass) {
        Write-Host "[CFG] boundary_class mismatch: source=$($t.source) target=$($t.target) expected=$expectedBoundaryClass actual=$boundaryClassText"
        $fail++
      }
    } elseif (-not [string]::IsNullOrWhiteSpace($boundaryClassText)) {
      if (-not (Test-BoundaryClassValue -BoundaryClass $boundaryClassText)) {
        Write-Host "[CFG] invalid boundary_class: expected one of global-user/project/shared-template source=$($t.source) target=$($t.target) actual=$boundaryClassText"
        $fail++
      } elseif ($boundaryClassText -ne $expectedBoundaryClass) {
        Write-Host "[CFG] boundary_class mismatch: source=$($t.source) target=$($t.target) expected=$expectedBoundaryClass actual=$boundaryClassText"
    ...
    $boundaryCheck = Get-BoundaryMappingCheck -Source ([string]$t.source) -Target ([string]$t.target) -RepoRoots $repoRoots
    if (-not $boundaryCheck.allowed) {
      Write-Host ("[CFG] boundary violation: source={0} target={1} reason={2} source_layer={3} target_layer={4}" -f $t.source, $t.target, $boundaryCheck.reason, $boundaryCheck.source_layer, $boundaryCheck.target_layer)
      $fail++
    }

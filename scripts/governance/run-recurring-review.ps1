...
function Write-AlertSnapshot([object]$ReviewResult, [string]$RootPath) {
  ...
  [void]$lines.Add(("runtime_agent_task_success_rate={0}" -f $ReviewResult.summary.runtime_agent_task_success_rate))
  [void]$lines.Add(("runtime_eval_pass_rate={0}" -f $ReviewResult.summary.runtime_eval_pass_rate))
  [void]$lines.Add(("runtime_cache_hit_rate={0}" -f $ReviewResult.summary.runtime_cache_hit_rate))
  [void]$lines.Add(("runtime_cost_per_successful_run={0}" -f $ReviewResult.summary.runtime_cost_per_successful_run))
  [void]$lines.Add(("runtime_tool_error_rate={0}" -f $ReviewResult.summary.runtime_tool_error_rate))
  [void]$lines.Add(("runtime_compaction_count={0}" -f $ReviewResult.summary.runtime_compaction_count))
  [void]$lines.Add(("runtime_trajectory_coverage_rate={0}" -f $ReviewResult.summary.runtime_trajectory_coverage_rate))
  [void]$lines.Add(("runtime_replay_ref_coverage_rate={0}" -f $ReviewResult.summary.runtime_replay_ref_coverage_rate))
  [void]$lines.Add(("runtime_checkpoint_ref_coverage_rate={0}" -f $ReviewResult.summary.runtime_checkpoint_ref_coverage_rate))
  [void]$lines.Add(("runtime_human_interrupt_count={0}" -f $ReviewResult.summary.runtime_human_interrupt_count))
  [void]$lines.Add(("runtime_eval_freshness_days={0}" -f $ReviewResult.summary.runtime_eval_freshness_days))
  [void]$lines.Add(("runtime_eval_missing_blocks_promotion={0}" -f $ReviewResult.summary.runtime_eval_missing_blocks_promotion))
  [void]$lines.Add(("runtime_trace_grading_enabled={0}" -f $ReviewResult.summary.runtime_trace_grading_enabled))
...
$runtimeAgentTaskSuccessRate = "N/A"
$runtimeEvalPassRate = "N/A"
$runtimeCacheHitRate = "N/A"
$runtimeCostPerSuccessfulRun = "N/A"
$runtimeToolErrorRate = "N/A"
$runtimeCompactionCount = "N/A"
$runtimeTrajectoryCoverageRate = "N/A"
$runtimeReplayRefCoverageRate = "N/A"
$runtimeCheckpointRefCoverageRate = "N/A"
$runtimeHumanInterruptCount = "N/A"
$runtimeEvalFreshnessDays = "N/A"
$runtimeEvalMissingBlocksPromotion = "N/A"
$runtimeTraceGradingEnabled = "N/A"
...
if ($metrics.exit_code -eq 0) {
  ...
  if (Test-Path -LiteralPath $metricsAutoPath -PathType Leaf) {
    ...
    $runtimeAgentTaskSuccessRate = Get-MetricValueFromFile -Path $metricsAutoPath -Key "agent_task_success_rate"
    if ([string]::IsNullOrWhiteSpace($runtimeAgentTaskSuccessRate)) { $runtimeAgentTaskSuccessRate = "N/A" }
    $runtimeEvalPassRate = Get-MetricValueFromFile -Path $metricsAutoPath -Key "runtime_eval_pass_rate"
    if ([string]::IsNullOrWhiteSpace($runtimeEvalPassRate)) { $runtimeEvalPassRate = "N/A" }
    $runtimeCacheHitRate = Get-MetricValueFromFile -Path $metricsAutoPath -Key "cache_hit_rate"
    if ([string]::IsNullOrWhiteSpace($runtimeCacheHitRate)) { $runtimeCacheHitRate = "N/A" }
    $runtimeCostPerSuccessfulRun = Get-MetricValueFromFile -Path $metricsAutoPath -Key "cost_per_successful_run"
    if ([string]::IsNullOrWhiteSpace($runtimeCostPerSuccessfulRun)) { $runtimeCostPerSuccessfulRun = "N/A" }
    $runtimeToolErrorRate = Get-MetricValueFromFile -Path $metricsAutoPath -Key "tool_error_rate"
    if ([string]::IsNullOrWhiteSpace($runtimeToolErrorRate)) { $runtimeToolErrorRate = "N/A" }
    $runtimeCompactionCount = Get-MetricValueFromFile -Path $metricsAutoPath -Key "compaction_count"
    if ([string]::IsNullOrWhiteSpace($runtimeCompactionCount)) { $runtimeCompactionCount = "N/A" }
    $runtimeTrajectoryCoverageRate = Get-MetricValueFromFile -Path $metricsAutoPath -Key "runtime_trajectory_coverage_rate"
    if ([string]::IsNullOrWhiteSpace($runtimeTrajectoryCoverageRate)) { $runtimeTrajectoryCoverageRate = "N/A" }
    $runtimeReplayRefCoverageRate = Get-MetricValueFromFile -Path $metricsAutoPath -Key "runtime_replay_ref_coverage_rate"
    if ([string]::IsNullOrWhiteSpace($runtimeReplayRefCoverageRate)) { $runtimeReplayRefCoverageRate = "N/A" }
    $runtimeCheckpointRefCoverageRate = Get-MetricValueFromFile -Path $metricsAutoPath -Key "runtime_checkpoint_ref_coverage_rate"
    if ([string]::IsNullOrWhiteSpace($runtimeCheckpointRefCoverageRate)) { $runtimeCheckpointRefCoverageRate = "N/A" }
    $runtimeHumanInterruptCount = Get-MetricValueFromFile -Path $metricsAutoPath -Key "runtime_human_interrupt_count"
    if ([string]::IsNullOrWhiteSpace($runtimeHumanInterruptCount)) { $runtimeHumanInterruptCount = "N/A" }
    $runtimeEvalFreshnessDays = Get-MetricValueFromFile -Path $metricsAutoPath -Key "runtime_eval_freshness_days"
    if ([string]::IsNullOrWhiteSpace($runtimeEvalFreshnessDays)) { $runtimeEvalFreshnessDays = "N/A" }
    $runtimeEvalMissingBlocksPromotion = Get-MetricValueFromFile -Path $metricsAutoPath -Key "runtime_eval_missing_blocks_promotion"
    if ([string]::IsNullOrWhiteSpace($runtimeEvalMissingBlocksPromotion)) { $runtimeEvalMissingBlocksPromotion = "N/A" }
    $runtimeTraceGradingEnabled = Get-MetricValueFromFile -Path $metricsAutoPath -Key "runtime_trace_grading_enabled"
    if ([string]::IsNullOrWhiteSpace($runtimeTraceGradingEnabled)) { $runtimeTraceGradingEnabled = "N/A" }
...
$result = [pscustomobject]@{
  ...
  summary = [pscustomobject]@{
    ...
    runtime_agent_task_success_rate = $runtimeAgentTaskSuccessRate
    runtime_eval_pass_rate = $runtimeEvalPassRate
    runtime_cache_hit_rate = $runtimeCacheHitRate
    runtime_cost_per_successful_run = $runtimeCostPerSuccessfulRun
    runtime_tool_error_rate = $runtimeToolErrorRate
    runtime_compaction_count = $runtimeCompactionCount
    runtime_trajectory_coverage_rate = $runtimeTrajectoryCoverageRate
    runtime_replay_ref_coverage_rate = $runtimeReplayRefCoverageRate
    runtime_checkpoint_ref_coverage_rate = $runtimeCheckpointRefCoverageRate
    runtime_human_interrupt_count = $runtimeHumanInterruptCount
    runtime_eval_freshness_days = $runtimeEvalFreshnessDays
    runtime_eval_missing_blocks_promotion = $runtimeEvalMissingBlocksPromotion
    runtime_trace_grading_enabled = $runtimeTraceGradingEnabled
...
Write-Host ("runtime_agent_task_success_rate={0}" -f $result.summary.runtime_agent_task_success_rate)
Write-Host ("runtime_eval_pass_rate={0}" -f $result.summary.runtime_eval_pass_rate)
Write-Host ("runtime_cache_hit_rate={0}" -f $result.summary.runtime_cache_hit_rate)
Write-Host ("runtime_cost_per_successful_run={0}" -f $result.summary.runtime_cost_per_successful_run)
Write-Host ("runtime_tool_error_rate={0}" -f $result.summary.runtime_tool_error_rate)
Write-Host ("runtime_compaction_count={0}" -f $result.summary.runtime_compaction_count)
Write-Host ("runtime_trajectory_coverage_rate={0}" -f $result.summary.runtime_trajectory_coverage_rate)
Write-Host ("runtime_replay_ref_coverage_rate={0}" -f $result.summary.runtime_replay_ref_coverage_rate)
Write-Host ("runtime_checkpoint_ref_coverage_rate={0}" -f $result.summary.runtime_checkpoint_ref_coverage_rate)
Write-Host ("runtime_human_interrupt_count={0}" -f $result.summary.runtime_human_interrupt_count)
Write-Host ("runtime_eval_freshness_days={0}" -f $result.summary.runtime_eval_freshness_days)
Write-Host ("runtime_eval_missing_blocks_promotion={0}" -f $result.summary.runtime_eval_missing_blocks_promotion)
Write-Host ("runtime_trace_grading_enabled={0}" -f $result.summary.runtime_trace_grading_enabled)

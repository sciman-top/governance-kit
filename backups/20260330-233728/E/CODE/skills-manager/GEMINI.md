...
- 规则优先级：`GEMINI.override.md > GEMINI.md > fallback`（平台支持时）。
- override 仅用于短期排障，结论后必须清理并复测。

### B.2 诊断与留痕
- 诊断顺序：`gemini --version -> gemini --help -> (status/doctor 类命令，若支持)`。
- 状态/加载链命令不存在或失败时，按 B.4 记录 N/A，不阻断后续仓内门禁。
- 证据最小字段：`cmd`、`exit_code`、`key_output`、`timestamp`、`active_rule_path`、`source`。
- `active_rule_path` 默认记录：`E:/CODE/skills-manager/GEMINI.md`（当 CLI 未显式提供加载链时）。

### B.3 平台能力剖面
- 不强制假定 Gemini CLI 提供统一状态命令。

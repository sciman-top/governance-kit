...
### B.2 最小诊断矩阵（执行序列）
- 固定顺序：`gemini --version -> gemini --help`。
- 能力探针（按 `gemini --help` 输出判定是否支持）：支持则执行 `gemini mcp --help`、`gemini skills --help`；不支持按 B.4 记 N/A。
- 留痕最低字段：`cmd`、`exit_code`、`key_output`、`timestamp`。

### B.3 平台能力剖面
- 状态能力：不可强制假定统一入口，按能力探针结果执行。
- 加载链可观测性：CLI 未显式展示加载链时，补记 `active_rule_path` 与来源。
- policy 可观测性：若使用 `--policy/--admin-policy`，需记录实际策略文件路径。

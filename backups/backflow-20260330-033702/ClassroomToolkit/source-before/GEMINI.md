...
### C.3 门禁命令存在性与 N/A 替代验证
- 执行前最小判定：`Get-Command dotnet`、`Test-Path ClassroomToolkit.sln`、`Test-Path tests/ClassroomToolkit.Tests/ClassroomToolkit.Tests.csproj`、`Test-Path scripts/quality/check-hotspot-line-budgets.ps1`。
- 若 `hotspot` 脚本缺失：`hotspot=N/A`，替代为执行 `contract/invariant` 子集测试并在证据中记录热点人工检查结论。
- 若 `contract/invariant` 子集不可执行：`contract/invariant=N/A`，替代为全量 `dotnet test` 并在证据中记录契约差异风险。
...
### C.7 目标仓直改回灌策略
- 规则源归宿：`E:/CODE/governance-kit/source/project/ClassroomToolkit/*`。
- 允许在目标仓临时直改用于快速试错，但必须在同一工作日回灌到规则源并留痕。
- 回灌后执行：`powershell -File E:/CODE/governance-kit/scripts/install.ps1 -Mode safe`，确保 `source` 与 `target` 再次一致。
- 未回灌前，禁止再次执行 `sync/install` 以免覆盖未沉淀改动。
## D. 维护校验清单（项目级）
- 仅落地本仓事实，不复写全局规则正文。
- 与 `GlobalUser/GEMINI.md` 职责互补，不重叠、不缺失。

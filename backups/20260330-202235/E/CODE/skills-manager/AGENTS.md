...
## 1. 阅读指引（必读）
- 本文件仅补充仓库差异；跨项目通用规则在 `GlobalUser/AGENTS.md`。
...
### A.2 协作与优先级
- 继承 `GlobalUser/AGENTS.md`，项目级规则优先于全局规则。
- 项目级只补充仓库差异，不复写全局共性条款。
- 规则来源：`GlobalUser/AGENTS.md` → `AGENTS.md`。

### A.3 N/A policy
- minimum fields: reason, alternative_verification, evidence_link.

## B. 平台差异（Codex 项目内）
- `targets` 包含 `.codex/skills` 时，需确认其指向或同步到 `agent/`。
- 拆分规则时遵循全局加载语义，避免同层多文件冲突。

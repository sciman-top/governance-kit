...
## 1. 阅读指引（必读）
- 本文件承接 `GlobalUser/AGENTS.md`，只定义 Skills Manager 落地动作（WHERE/HOW）。
...
- 最小诊断链：`codex status` -> `codex --version` -> `codex --help`。
- 若 `codex status` 因非交互环境不可用，需记录 N/A 原因和替代证据。

### B.2 目录同步差异
- 若 `skills.json.targets` 含 `.codex/skills`，验证其与 `agent/` 同步状态。

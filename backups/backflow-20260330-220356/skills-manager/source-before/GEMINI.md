...
## 1. 阅读指引（必读）
- 本文件承接 `GlobalUser/GEMINI.md`，仅定义 Skills Manager 落地动作（WHERE/HOW）。
...
## B. Gemini 平台差异（项目内）
### B.1 加载与诊断
- 规则优先级：`GEMINI.override.md > GEMINI.md > fallback`。
- 最小诊断链：`gemini --version -> gemini --help`（状态命令若支持则补充）。
- 若命令不可用，记录 N/A 原因与替代证据。

### B.2 目录同步差异
- 若 `skills.json.targets` 含 `.gemini/skills`，需验证其与 `agent/` 同步状态。

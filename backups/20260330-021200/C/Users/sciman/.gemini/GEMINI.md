# GEMINI.md — Universal Agent Protocol v9.31
...
## B. Gemini 平台差异（全局）
### B.1 加载链与覆盖
- 文件名优先级：`GEMINI.override.md > GEMINI.md > fallback`（平台支持时生效）。
- 推荐用户目录：`~/.gemini`；以本机 CLI 实际加载结果为准。
- `GEMINI.override.md` 仅用于短期排障；任务结束后删除并复测。

### B.2 最小诊断动作
1. `gemini --version`（记录运行时版本）。
2. `gemini --help`（核对子命令可用性）。

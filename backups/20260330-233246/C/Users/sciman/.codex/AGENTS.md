# AGENTS.md — Universal Agent Protocol v9.36
...
- 必须写明 `reason`、`alternative_verification`、`evidence_link`。
...
- 状态检查优先执行 `codex status`；若在非交互终端失败（如 `stdin is not a terminal`），按 B.4 记录 N/A。
- 留痕最低字段：`cmd`、`exit_code`、`key_output`、`timestamp`。

### B.3 平台能力剖面（Codex）
- 状态命令能力：`codex status` 为首选状态入口。
- 加载链可观测性：若 `status` 不提供加载链，必须补记 `active_rule_path` 与来源。
- override 能力：若当前 CLI 版本不支持，按 `N/A + alternative_verification` 记录。

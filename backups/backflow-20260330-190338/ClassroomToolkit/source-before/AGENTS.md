# AGENTS.md — ClassroomToolkit 项目规则
**项目**: ClassroomToolkit  
...
## 1. 阅读指引（项目级）
- 本文件承接 $GlobalRef，仅定义本仓落地动作（WHERE/HOW）。
- 固定结构：1 / A / B / C / D。
...
### A.3 N/A 策略
- 仅在命令/脚本客观不存在时允许 N/A。
- 最低字段：eason、lternative_verification、vidence_link。
- N/A 不得改变门禁语义顺序：uild -> test -> contract/invariant -> hotspot。
...
- 	ests/ClassroomToolkit.Tests：xUnit + FluentAssertions（回归与契约主阵地）。
...
- 固定顺序：uild -> test -> contract/invariant -> hotspot。
...
### C.4 失败分流与阻断
- uild 失败：阻断，先修编译错误与引用断裂。
- 	est 失败：阻断，先修回归或错误用例，再重跑全链路。
- contract/invariant 失败：高风险阻断，禁止合并/发布。
...
- Waiver 键：owner、xpires_at、status、ecovery_plan、vidence_link。
...
- 回灌后必须执行：install.ps1 -Mode safe + doctor.ps1，确认 source/target 一致。
...
- Azure Pipelines: zure-pipelines.yml
...
- templates: docs/change-evidence/template.md, docs/governance/waiver-template.md, docs/governance/metrics-template.md present

...
- 扩展能力采用“先探测后调用”：`gemini --help | Select-String "mcp|extensions|skills|hooks|approval-mode|sandbox|policy"`（可见再执行）。
...
- `--allowed-tools` 标记为 deprecated 时仅做兼容，不作为治理基线；优先 `policy/admin-policy`。

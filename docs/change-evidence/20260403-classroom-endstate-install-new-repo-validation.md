...
目标归宿=governance-kit installer can bootstrap ClassroomToolkit custom governance stack into new repos
...
- governance-kit health: validate-config PASS, verify PASS(ok=29), doctor HEALTH=GREEN.
...
- Revert governance-kit changes in:
...
- Restore previous snapshot via governance-kit backups and rerun:
  powershell -NoProfile -ExecutionPolicy Bypass -File scripts/doctor.ps1

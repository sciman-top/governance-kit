...
| `gate.evidence_template_fields` | gate | `scripts/governance/check-evidence-template-fields.ps1`, `docs/change-evidence/template.md` | `source_only` | `progressive` | `observe` | `balanced` | Ensures evidence template keeps mandatory Phase2 decision and friction fields. |
| `gate.target_rollout_matrix_gap` | gate | `scripts/governance/check-target-rollout-matrix.ps1`, `config/target-control-rollout-matrix.json` | `source_only` | `progressive` | `observe` | `balanced` | Ensures each distributable progressive control has per-target rollout state coverage. |
...
| `review.target_rollout_status_matrix` | review_evolution | `docs/governance/target-rollout-status-matrix-2026Q2.md` | `source_only` | `progressive` | `observe` | `balanced` | Provides explicit cross-repo rollout matrix for progressive distributable controls. |
